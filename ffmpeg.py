...
def create_videos(
    ...
    if avg_runtime != -1:
        ...
        # seconds
        print(
            f"\033[0;32mEstimated run time: ",
            round(estimated_runtime, 2),
            " seconds\033[0m",
        )
        ...
        if round(estimated_runtime / 60, 2) > 60:  # hours
            print(
                "\033[0;32m = ",
                round((estimated_runtime / 60) / 60, 2),
                " hours\033[0m",
            )
    ...
    for i in range(number_of_videos):
        ...
        file_name = f"{i}-{to_snake_case(text_source_for_name[:10])}_{random_video_num}_{random_audio_num}_{random_font_num}.mp4"
    ...
    if number_of_videos > 1:
        ...
        print(
            ...
            f"\nTotal run time:",
            round(run_time_total, 2),
            "seconds = ",
            round(run_time_total / 60, 2),
            " minutes!",
            f"\nAverage run time:",
            round(run_time_average, 2),
            "seconds! \033[0m",
        )
...
def create_video(
    ...
    validate_file_path(audio_file)
    validate_file_path(video_file)
    validate_file_path(created_verse_image)
    validate_file_path(text_source_font)
    ...
    if posts:
        verse_handler.create_post_images(
            video_path=output_path, output_folder=f"{output_folder}/post_images"
        )
