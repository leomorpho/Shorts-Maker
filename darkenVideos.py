...
def generate_darken_video(video_file, output_path):
    video_clip = vfc.VideoFileClip(video_file, audio=False).without_audio()

    # Save the final video
    darken_clip = video_clip.fl_image(darken)
    darken_clip.write_videofile(output_path, threads=8, codec="libx264")
...
def cut_vertical_to_horizontal(video_folder, output_folder):
    ...
    for video_file in video_files:
        ...
        cmd = [
            ...
            video_file,
            "-vf",
            f"crop={crop_width}:{crop_height}:(in_w-{crop_width})/2:(in_h-{crop_height})/2,scale=1080:1920",
            "-c:a",
            "copy",
            out,
        ]
